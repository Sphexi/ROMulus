...
#
# User-editable data folders (``dats/``, ``gamedb/``,
# ``libretro-metadat/``, ``profiles/``, ``systems/``) are NOT embedded
# in the exe — ``build-portable.ps1`` copies them next to the exe in
# the final ZIP so users can see and edit them on first extract without
# having to launch the app to trigger seeding. End-user layout::
#
#   romulus.exe                       (single self-contained binary)
#   dats/*.dat                        (bundled No-Intro DAT files)
#   gamedb/*.json                     (bundled GameDB snapshots)
#   libretro-metadat/<dim>/*.dat      (bundled libretro-database)
#   profiles/*.yaml                   (destination profiles)
#   systems/*.yaml                    (system registry YAMLs)
#
# Build locally with::
#
